...
public class Sm4 {
    ...
    private byte[] padding(byte[] input, int mode) {
        ...
        if (mode == SM4_ENCRYPT) {
            ...
            // 补位验证
            if (p <= 0 || p > LENGTH_SIXTEEN) {
                return input;
            }
            if (p > 0 && p <= LENGTH_SIXTEEN) {
                for (int i = 0; i < p; i++) {
                    if (input[input.length - 1 - i] != p) {
                        return input;
                    }
                }
            }
    ...
    public void sm4SetkeyEnc(Sm4Context ctx, byte[] key) throws Exception {
        ...
        if (key == null || key.length != LENGTH_SIXTEEN) {
    ...
    public byte[] sm4CryptEcb(Sm4Context ctx, byte[] input) throws Exception {
        ...
        try (ByteArrayInputStream bins = new ByteArrayInputStream(input); ByteArrayOutputStream bous = new ByteArrayOutputStream()) {
            int length = input.length;
            for (; length > 0; length -= LENGTH_SIXTEEN) {
                byte[] in = new byte[16];
    ...
    public byte[] sm4CryptCbc(Sm4Context ctx, byte[] iv, byte[] input) throws Exception {
        if (iv == null || iv.length != LENGTH_SIXTEEN) {
